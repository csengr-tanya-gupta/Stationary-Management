...
public class InventoryController {
    ...
    public ResponseEntity<List<StationeryItemResponse>> getLowStockItems(
        ...
        // Only admins can view low stock alerts. This helps the purchasing department 
        // know what items need to be restocked to fulfill upcoming requests.
        if (!"ADMIN".equalsIgnoreCase(userRole)) {
    ...
    public ResponseEntity<Boolean> deductQuantity(
        ...
        // This endpoint is internal to the backend cluster. It is called synchronously by the 
        // Request Service via a Feign Client when an admin approves a stationery request.
        // It immediately reduces available stock in the database. If stock is insufficient, 
        // this method throws an exception (HTTP 400), which cascades back to Request Service
        // and aborts the entire request approval transaction.
